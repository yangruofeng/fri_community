...
function global_exception_handler(Exception $e)
{
    ...
    $rt = new result(false,$e->getMessage(),$e,0);
...
class classLoader
{
    ...
    {
        if (self::$_class_map === NULL) {
            if (file_exists(_CACHE_ . "/class_map.tmp")) {
                include_once(_CACHE_ . "/class_map.tmp");
                //$map_file_json=file_get_contents(_ROOT_."/class_map.json");
                self::$_class_map = $class_map;
            } else {
                self::$_class_map = array();
            }
        }
        //simple and works, todo later upgrade the "ClassPath"
        if (self::$_class_map[$className]) {
            $_fname = self::$_class_map[$className];
            $_fname = GLOBAL_ROOT . "/" . $_fname;
        ...
        if (strlen($class) > 7 && ucwords(substr($class, -7)) == 'Control') {
            if (self::find_file($className, CURRENT_ROOT . DS . "control", substr($class, 0, -7))) return;
            $class_path_a = array(CURRENT_ROOT . DS . "control");
        } elseif (strlen($class) > 5 && ucwords(substr($class, -5)) == 'Model') {
            if (self::find_file($className, _DATA_MODEL_, substr($class, 0, -5) . ".model")) return;
            $class_path_a = array(_DATA_MODEL_);
            if (self::find_file($className, BASE_MODEL_PATH, substr($class, 0, -5) . ".model")) return;
            $class_path_a[] = BASE_MODEL_PATH;
        } elseif (strlen($class) > 5 && ucwords(substr($class, -5)) == 'Class') {
            if (self::find_file($className, PROJECT_ROOT . DS . "class", substr($class, 0, -5) . ".class")) return;
            $class_path_a = array(PROJECT_ROOT . DS . "class");
        } elseif (strlen($class) > 3 && ucwords(substr($class, -3)) == 'Api') {
            if (self::find_file($className, BASE_CORE_PATH . DS . "api", substr($class, 0, -3) . ".api")) return;
            if (self::find_file($className, BASE_DATA_PATH . DS . "api", substr($class, 0, -3) . ".api")) return;
            $class_path_a = array(
            ...
            if (defined("BASE_COMMON_PATH")) {
                if (self::find_file($className, BASE_COMMON_PATH . DS . "api", substr($class, 0, -3) . ".api")) return;
        ...
        foreach ($class_path_a as $path) {
            if (self::find_file($className, $path)) {
                return;
            }
            if (self::find_file(strtolower($className), $path)) {
                return;
            }
            if( self::find_file(strtolower(self::decamelize($className)),$path) ){
                return;
            }
            if( self::find_file(self::decamelize($className),$path) ){
                return;
            }
        }
    ...
    static function find_file($className, $path, $filename = null)
    {
        ...
        if (file_exists($path . "/$filename.php")) {
            ...
            self::$_class_map[$className] = $_fname;
            $file_s = var_export(self::$_class_map, true);
            $file_s = "\$class_map=$file_s;";
            file_put_contents(_CACHE_ . "/class_map.tmp", "<" . "?php\n$file_s");
            return $_fname;
        } else {
 ...
 */
function showMessage($msg, $url = '', $time = 2000)
{
...
function getUserIcon($icon = '', $dir = 'avatar')
{
    ...
    $img_src = _UPLOAD_ . DS . $dir . DS . $icon;
    if (file_exists($img_src)) {
        return UPLOAD_SITE_URL . DS . $dir . DS . $icon;
    } else {
        return ENTRY_DESKTOP_SITE_URL . DS . 'resource/img/default_avatar.jpg';
    }
}
