...
class userClass
{
    ...
    public function addUser($param)
    {
        ...
        $m_um_user = M('um_user');
        $m_um_role_role = M('um_user_role');
        $m_um_user_position = M('um_user_position');
        ...
        try {
            ...
            foreach ($user_position as $position) {
                $row_position = $m_um_user_position->newRow();
                $row_position->user_id = $rt_1->AUTO_ID;
                $row_position->user_position = $position;
                $rt_7 = $row_position->insert();
                if (!$rt_7->STS) {
                    $conn->rollback();
                    return new result(false, 'Add failed--' . $rt_7->MSG);
                }
            }
    ...
    public function editUser($param)
    {
        ...
        try {
            ...
            $rt_8 = $m_um_user_position->delete(array('user_id' => $uid));
            if (!$rt_8->STS) {
                $conn->rollback();
                return new result(false, 'Edit failed--' . $rt_8->MSG);
            }
            foreach ($user_position as $position) {
                $row_position = $m_um_user_position->newRow();
                $row_position->user_id = $uid;
                $row_position->user_position = $position;
                $rt_7 = $row_position->insert();
                if (!$rt_7->STS) {
                    $conn->rollback();
    ...
    public function deleteUser($uid)
    {
        ...
        $m_um_user_role = M('um_user_role');
        $m_special_auth = M('um_special_auth');
        $m_um_position_task = M('um_position_task');
        ...
        try {
            ...
            $rt_4 = $m_um_position_task->delete(array('user_id' => $uid));
            if (!$rt_4->STS) {
                $conn->rollback();
                return new result(false, 'Delete failed--' . $rt_4->MSG);
            }
