...
<!--            <td>--><?php //echo 'Handle Time'; ?><!--</td>-->
        ...
        <?php foreach ($data['data'] as $row) { ?>
            <tr>
                <td>
                    <a href="<?php echo getUrl('loan', 'contractDetail', array('uid'=>$row['contract_id']), false, BACK_OFFICE_SITE_URL); ?>">
...
<!--                    --><?php //}?>
                    <?php if($row['state'] == prepaymentApplyStateEnum::CREATE || $row['state'] == prepaymentApplyStateEnum::AUDITING) {?>
                        <div class="custom-btn-group">
                            <a title="" class="custom-btn custom-btn-secondary" href="<?php echo getUrl('loan', 'auditRequestPrepayment', array('uid' => $row['uid']), false, BACK_OFFICE_SITE_URL) ?>">
                                <span><i class="fa fa-check-circle-o"></i>Handle</span>
                            </a>
                        </div>
                    <?php }?>
                    <?php if(!in_array($row['state'],array(prepaymentApplyStateEnum::CREATE,prepaymentApplyStateEnum::AUDITING))){ ?>
                        <div class="custom-btn-group">
                            <a title="" class="custom-btn custom-btn-secondary" href="<?php echo getUrl('loan', 'viewRequestPrepayment', array('uid' => $row['uid']), false, BACK_OFFICE_SITE_URL) ?>">
