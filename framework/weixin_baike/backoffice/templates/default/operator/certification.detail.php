...
<div class="page">
    <div class="fixed-bar">
        <div class="item-title">
            ...
            <ul class="tab-base">
                <li>
                    <a href="<?php echo getUrl('operator', 'certificationFile', array('cert_type' => $info['cert_type']), false, BACK_OFFICE_SITE_URL) ?>"><span><?php echo $output['title'] . ' List'?></span></a>
                </li>
        ...
        <form class="demoform" id="validIDForm" method="post" action="<?php echo getUrl('operator', 'certificationConfirm', array(), false, BACK_OFFICE_SITE_URL) ?>">
            ...
            <table class="table">
                            ...
                            <?php foreach ($output['cert_sample_images'][$info['cert_type']] as $sample) { ?>
                                <div style="display:inline-block;width: 200px;text-align: center;margin-right: 5px;">
                                    <a target="_blank" href="<?php echo $sample['image']; ?>">
                                        <img src="<?php echo $sample['image']; ?>" style="width: 200px;height: 200px"/>
                                    </a>
                                    <h5 style="color:red;">
                                        <?php echo $sample['des']; ?>
                                    </h5>
                    ...
                    <td>
                        <?php foreach ($info['cert_images'] as $value) { ?>
                            <a target="_blank" href="<?php echo $value['image_url']; ?>">
                                <img src="<?php echo $value['image_url']; ?>" class="verify-img" alt=""></a>
                        <?php } ?>
                    </td>
                </tr>
                <?php if ($info['cert_type'] == certificationTypeEnum::ID) { ?>
                ...
                <?php if ($info['cert_type'] == certificationTypeEnum::FAIMILYBOOK) { ?>
                    <tr>
                        <td><label class="control-label">Cert Name</label></td>
                        <td><?php echo $IDInfo['cert_name']; ?></td>
                    </tr>
                    <tr>
                        <td><label class="control-label">Cert Sn</label></td>
                        <td><?php echo $IDInfo['cert_sn']; ?></td>
                    </tr>
                    <tr>
                        <td><label class="control-label">Cert Address</label></td>
                        <td><?php echo $IDInfo['cert_addr']; ?></td>
                    </tr>
                    <tr>
                        <td><label class="control-label">Cert Expire Time</label></td>
                        <td><?php echo dateFormat($IDInfo['cert_expire_time']); ?></td>
                    </tr>
                <?php } ?>
                <tr>
                    <td><label class="control-label">Source Type</label></td>
                    <td><?php if ($info['source_type'] == 0) {
                ...
                <tr>
                    ...
                    <td><?php if ($info['verify_state'] == -1) {
                            echo 'Auditing...';
                        } elseif ($info['verify_state'] == 0) {
                            echo 'Not Verified';
                        } elseif ($info['verify_state'] == 10) {
                            echo 'Passed';
                        } else {
                            echo 'Not Pass';
                        } ?>
                    ...
                    <td>
                        <?php if ($info['verify_state'] == 10) { ?>
                            ...
                            <div class="custom-btn-group approval-btn-group">
                                <button type="button" class="btn btn-info" onclick="javascript:history.go(-1);"><i
                        ...
                        <?php } else { ?>
                            <div class="custom-btn-group approval-btn-group">
                                ...
                                <button type="button" class="btn btn-info" onclick="checkAbandon();"><i class="fa fa-close"></i><?php echo 'Abandon' ?></button>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php foreach ($row['cert_images'] as $value) { ?>
                                        <span>
                                            <img src="<?php echo $value['image_url']; ?>" style="width: 80px;height: 70px;">
                                        </span>
                                    <?php } ?>
                                </td>
                                <td><?php echo $row['cert_name'] ?></td>
                                <td><?php echo $row['cert_sn'] ?></td>
                                <td><?php if ($row['verify_state'] == 0) {
                                        echo 'Not Verified';
                                    } elseif ($row['verify_state'] == 10) {
                                        echo 'Have Passed';
                                    } elseif ($row['verify_state'] == 100) {
                                        echo 'Refuse';
                                    } else {
                                        echo 'Audit...';
                                    } ?></td>
                                <td><?php if ($row['cert_type'] == certificationTypeEnum::ID) {
                                        echo 'ID';
                                    } elseif ($row['cert_type'] == certificationTypeEnum::FAIMILYBOOK) {
                                        echo 'Faimily Book';
                                    } elseif ($row['cert_type'] == certificationTypeEnum::PASSPORT) {
                                        echo 'Passport';
                                    } elseif ($row['cert_type'] == certificationTypeEnum::HOUSE) {
                                        echo 'House';
                                    } elseif ($row['cert_type'] == certificationTypeEnum::CAR) {
                                        echo 'Car';
                                    } ?></td>
...
<script>
    ...
    function checkAbandon(){
        ...
        yo.loadData({
            _c: 'operator',
            _m: 'abandonCertificationFile',
            param: {uid: uid},
            callback: function (_o) {
                if (_o.STS) {
                    alert(_o.MSG);
                    window.location.href = '<?php echo getUrl('operator', 'certificationFile', array('cert_type' => $info['cert_type']), false, BACK_OFFICE_SITE_URL);?>';
                } else {
                    alert(_o.MSG);
                }
            }
